...
function partialJsonValidate(request, doc, part, schema) {
  ...
  return connection
    ...
    .then(
      coll => (collection = coll) && collection.insert(doc),
      error => onError('Error getting collection: ', error),
    )
    .then(
      result => collection.updateOne(spec, part),
      error => onError('Error inserting into validation collection: ', error),
    )
    .then(
      result => collection.find(spec).toArray(),
      error => onError('Error updating in validation collection: ', error),
    )
    .then(
      doc => validate(doc[0]),
      error => onError('Error finding updated document in validation collection: ', error),
    )
