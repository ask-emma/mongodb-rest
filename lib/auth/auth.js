...
module.exports = function(app, config) {
  ...
  if (!config.auth.authDB) {
    throw new Error(
      "Authentication has been enabled by including 'auth' in the configuration, but users database connection string 'authDB' has not been specified.",
    )
  }
  ...
  app.use(function(req, res, next) {
    ...
    if (!token) {
      logger.warn('Unauthorized access - no token supplied')
    ...
    if (config.auth.universalAuthToken && token === config.auth.universalAuthToken) {
      logger.verbose('Access authorized by use of universal auth token')
      next()
      return
    }

    const authDBConnection = `${config.db}/${config.auth.authDB}`
    connection.connect(authDBConnection, function(err, authDb) {
      if (err) {
        logger.error('Failed to connect to database ' + authDBConnection + ': ' + err.message)
        res.status(500).json({ error: err })
        return
      }

      authDb.collection(tokensCollectionName, function(err, tokensCollection) {
        if (err) {
        ...
        tokensCollection.findOne({ token: token }, function(err, token) {
          ...
          if (!token) {
            logger.warn('Unauthorized access - token ' + token + ' not found in database')
  ...
  app.use('/login', function(req, res) {
    ...
    if (!password) {
      ...
      logger.warn('Request error: ' + msg)
      res.status(400).json({ message: msg })
      return
    }
    const authDBConnection = `${config.db}/${config.auth.authDB}`
    logger.info(authDBConnection)
    connection.connect(authDBConnection, function(err, authDB) {
      if (err) {
        logger.error('Failed to connect to database ' + authDBConnection + ': ' + err.message)
      ...
      authDB.collection(usersCollectionName, function(err, usersCollection) {
        if (err) {
