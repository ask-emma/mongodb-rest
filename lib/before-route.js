...
function setDbUri(request, response, config) {
  ...
  if (config.db.authSource) {
    request.dbUri += `?authSource=${config.db.authSource}`
  }
  return true
}
...
function checkDBAceess(request, response, config) {
  ...
  if (isDbEndpoint) {
    ...
    allow =
      typeof allowedCollections !== 'undefined' &&
      (typeof collection === 'undefined' || !allowedCollections.length || allowedCollections.indexOf(collection) > -1)
  }
