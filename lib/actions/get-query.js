...
function getQuery(request, response, config) {
  ...
  connection
    ...
    .then(
      cursor => applyOptions(cursor),
      error => onError('Error finding document(s): ', error),
    )
    .then(
  ...
  function applyOptions(cursor) {
    ...
    return o.explain ? cursor.explain() : cursor.toArray()
  }
  ...
  function buildQuery(request) {
    ...
    if (request.params.id) {
      ...
      query = normalizeNestedQuery(query)
    }
    return query
  }
  ...
  function normalizeNestedQuery(obj) {
    for (var key in obj) {
      ...
      if (typeof obj[key] === 'object') {
        obj[key] = normalizeNestedQuery(obj[key])
      }

      // Found a match
      if (key.indexOf('_id') > -1) {
        // Cast the id to an Object id
        obj[key] = castId(obj[key])
      } else if (key == '$regex') {
