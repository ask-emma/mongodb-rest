...
function patchUpdate(request, response, config) {
  ...
  connection
    .connectPromise(request.dbUri)
    .then(
      db => db.collection(request.params.collection),
      error => onError('Db open error: ', error),
    )
    .then(
      coll => (collection = coll) && collection.findOne(spec),
      error => onError('Error getting collection: ', error),
    )
    .then(
      doc => partialJsonValidate(request, doc, updateData, schema),
      error => onError('Error finding document: ', error),
    )
    .then(isValid => {
      if (!isValid) return response.status(400).json({ message: "Updating document won't match predefined schema" })

      return collection
        .updateOne(spec, updateData)
        .then(
          result => fetchUpdated(result),
          error => onError('Error performing update: ', error),
        )
        .then(
          doc => response.onDone(doc),
          error => onError('Error finding updated document: ', error),
        )
    })
  ...
  function fetchUpdated(result) {
    return result.result.n ? collection.findOne(spec) : []
  }
}
