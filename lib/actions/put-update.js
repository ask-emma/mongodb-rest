...
function putUpdate(request, response, config) {
  ...
  connection
    ...
    .then(
      coll => (collection = coll) && collection.updateOne(spec, request.body),
    ...
    .then(
      docs => response.onDone(docs[0]),
      error => onError('Error finding updated documents: ', error),
    )
    .catch(error => response.onError('', error))

  function fetchUpdated(result) {
    return result.result.n ? collection.find(spec).toArray() : []
  }
}
