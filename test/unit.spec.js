...
describe('mongodb-rest:unit', function() {
  function isDbEndpointProvider() {
    return [
      ...
      {
        note: 'sets an endpoint to "server" from "endpoint_root"',
        config: { endpoint_root: 'server' },
        expected: false,
      },
      {
        note: 'sets an endpoint to "database" from "endpoint_root"',
  ...
  function setDbParamByEndpointProvider() {
    return [
      {
        note: 'sets empty db request parameter for database endpoint mode',
        config: { endpoint_root: 'database' },
        expected: { params: { db: '' } },
      },
      {
        note: 'do not set empty db request parameter for server endpoint mode',
        config: { endpoint_root: 'server' },
        expected: { params: {} },
      },
      {
        note: 'do not set empty db request parameter if endpoint_root is not set',
        config: {},
        expected: { params: {} },
      },
    ]
  ...
  function castIdProvider() {
    return [
      ...
      {
        note: 'should cast hex id',
        id: '5b68a4c62e5cfd380d73ea48',
        expected: new ObjectID('5b68a4c62e5cfd380d73ea48'),
      },
    ]
  }
